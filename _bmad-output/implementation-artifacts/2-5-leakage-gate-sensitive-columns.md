...
| 2026-07-03 | Second-pass adversarial verification of the applied patches (all 5 fixes confirmed correct, no regressions). Fixed one latent MEDIUM it surfaced in the gate's core predicate: `_colliding_mask` used `Series.map`, which upcasts a nullable `Int64` column that holds a `pd.NA` to float (`200` → `"200.0"`), missing the guard's `"200"` digest — a false negative reachable via the public `enforce_leakage_gate` API (harmless in 2.5 since sensitive numerics are suppressed, but a live leak once Epic 4 generates them). Now `column.astype(object).map(...)` preserves each scalar's type with `pd.NA` intact. +1 regression test → 235 unit. |
