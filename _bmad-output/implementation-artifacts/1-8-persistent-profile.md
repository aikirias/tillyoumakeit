...
| 2026-07-02 | Full 3-layer `bmad-code-review` gate (retroactive). AC audit: 7/7 SATISFIED. Fixed 1 MEDIUM (flagged by **both** Blind Hunter and Edge Case Hunter): the `-o/--out` save path let an `OSError` (missing parent dir / permission denied) escape as a raw traceback, unlike the load path — now wrapped into a clean message + exit 1. +1 test. Dismissed (not reachable from our writer): `_int_freq` None→{}, hand-edited version-string equality, ragged-matrix shape validation. |
