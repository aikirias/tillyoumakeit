...
def reflect_schema(engine: Engine, table: str) -> Schema:
    """Reflect a table into the canonical Schema. Raises TableNotFoundError."""
    inspector = sa_inspect(engine)
    try:
        raw_columns = inspector.get_columns(table)
    except NoSuchTableError:
        raise TableNotFoundError(f"Table {table!r} not found.") from None
    if not raw_columns:
        raise TableNotFoundError(f"Table {table!r} not found.")

    pk_cols = tuple(
        _safe(lambda: inspector.get_pk_constraint(table), {}).get("constrained_columns", []) or []
    )
    columns = tuple(
        Column(
            name=col["name"],
    ...
    foreign_keys = tuple(
        ...
        for fk in _safe(lambda: inspector.get_foreign_keys(table), [])
    )
    unique_constraints = tuple(
        tuple(uc.get("column_names", []))
        for uc in _safe(lambda: inspector.get_unique_constraints(table), [])
    )
    indexes = tuple(
        Index(
            name=ix.get("name"),
        ...
        for ix in _safe(lambda: inspector.get_indexes(table), [])
    )
    return Schema(
        columns=columns,
        primary_key=pk_cols,
